...
`;
  ...
  const generateCustomStory = async () => {
    ...
    if (!currentUser?.id) {
      setError("Please log in to generate custom stories.");
      return;
    }

    // Check if user has enough tokens
    const tokensRequired = 5; // Set the cost for multiverse story generation
    ...
    if (!hasEnoughTokens) {
      setError(
        `Insufficient tokens. You need ${tokensRequired} tokens to generate a custom story. Please purchase more tokens to continue.`
      );
      return;
    }
    ...
    try {
      ...
      // Show success message with updated token count
      setError(
        `Story generated successfully! ${tokensRequired} tokens consumed. Remaining tokens: ${tokenResult.remainingTokens}`
      );
        ...
        {userProfile.tokens < 5 && (
          <div className="mt-3 p-3 bg-yellow-500/20 rounded-lg border border-yellow-500/20 flex items-center space-x-2">
            ...
            <span className="text-yellow-300 text-sm">
              Low tokens! You need 5 tokens to generate custom stories.
            </span>
          </div>
        )}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-black/40 backdrop-blur-xl rounded-lg p-6 border border-white/5">
          ...
          >
            {isLoading || tokenLoading ? (
              <>
                ...
                <span>
                  {tokenLoading ? "Consuming Tokens..." : "Generating..."}
                </span>
              </>
            ) : (
              <>
                <Sparkles className="w-4 h-4" />
                <span>Create Story (5 tokens)</span>
                {userProfile && userProfile.tokens < 5 && (
                  <Lock className="w-4 h-4 text-red-400" />
                )}
              </>
        ...
        {customStoryline.length > 0 && userProfile && (
          <div className="mt-4 p-4 bg-black/20 rounded-lg border border-white/10">
            <div className="flex items-center justify-between mb-2">
              <h4 className="text-white">Your Custom Storyline</h4>
              <div className="flex items-center space-x-2 text-sm">
                <Coins className="w-4 h-4 text-white/60" />
                <span className="text-white/60">Cost: 5 tokens</span>
                <span className="text-white/40">|</span>
                <span
                  className={`${
                    userProfile.tokens >= 5 ? "text-green-400" : "text-red-400"
                  }`}
                >
                  Available: {userProfile.tokens}
                </span>
              </div>
            </div>
            ...
            {userProfile.tokens < 5 && (
              <div className="mt-2 p-2 bg-red-500/20 rounded border border-red-500/20">
                <p className="text-red-300 text-sm flex items-center space-x-2">
                  <AlertTriangle className="w-4 h-4" />
                  <span>
                    Insufficient tokens. Please purchase more tokens to generate
                    stories.
                  </span>
                </p>
              </div>
